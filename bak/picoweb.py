...
class WebApp:
    ...
    def _handle(self, reader, writer):
        ...
        try:
            ...
            if self.debug >= 0:
                self.log.info('%.3f %s %s "%s %s"' % (time.time(), req, writer, method, path))
            path = path.split("?", 1)
            qs = ""
            if len(path) > 1:
                qs = path[1]
            ...
            if self.debug >= 0:
                self.log.exc(e, "%.3f %s %s %r" % (time.time(), req, writer, e))
            yield from self.handle_exc(req, writer, e)

        if close is not False:
            yield from writer.aclose()
        if __debug__ and self.debug > 1:
            self.log.debug("%.3f %s Finished processing request", time.time(), req)
    ...
    def run(self, host="127.0.0.1", port=8081, debug=False, lazy_init=False, log=None):
        ...
        if debug > 0:
            _log.ilog("HTTP Server Started on http://%s:%s/" % (host, port), 'lib.picoweb.run')
